...
abstract class GeneratorCommand extends Command
{
	...
	/**
	 * Qualify the given entity class base name.
	 *
	 * @param  string  $entity
	 * @return string
	 */
	protected function qualifyEntity(string $entity)
	{
		$entity = ltrim($entity, '\\/');
		...
		if (Str::startsWith($entity, $rootNamespace)) {
			return $entity;
		}
	...
	{
		$searches = [
			['DummyNamespace', 'DummyRootNamespace', 'NamespacedDummyUserEntity'],
			['{{ namespace }}', '{{ rootNamespace }}', '{{ namespacedUserEntity }}'],
			['{{namespace}}', '{{rootNamespace}}', '{{namespacedUserEntity}}'],
		];
		...
		foreach ($searches as $search) {
			$stub = str_replace(
				$search,
				[$this->getNamespace($name), $this->rootNamespace(), $this->userProviderEntity()],
				$stub
			);
		}
	...
	/**
	 * Get the entity for the default guard's user provider.
	 *
	 * @return string|null
	 */
